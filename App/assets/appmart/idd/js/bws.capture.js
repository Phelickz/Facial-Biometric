...
function getImageBrightness(imageSrc, callback) {
    ...
    img.onload = function() {
        ...
        var tabl = "<table ><thead><tr><th width='100%'> </th></tr></thead></table >";
        $("#div1").append(tabl);
        $("#div1").append(tbl);
...
(function(bws, $, undefined) {
    ...
    bws.initcapture = function(canvasElement, issuedToken, options) {
        ...
        if (typeof settings.host !== 'undefined') {
            settings.apiurl = 'https://' + settings.host + '/extension/';
        }
        ...
        var startRecording = function(challenges, countdown) {
            ...
            if (countdown && statusCallback) {
                ...
                setTimeout(function() {
                    statusCallback('UserInstruction-2');
                }, 400);
                setTimeout(function() {
                    statusCallback('UserInstruction-1');
                }, 800);
        ...
        function initRecording() {
            if (statusCallback) {
                statusCallback('UserInstruction-Start');
            }
            recording(true);
            startActivityTimer();
        }
        ...
        function startMotionTimer() {
            ...
            noMotionTimer = setInterval(function() {
                if (uploading + uploaded < settings.recordings) {
                    if (statusCallback) {
                        statusCallback('UserInstruction-NoMovement');
                    }
                }
            }, 5000);
        }
        ...
        function upload() {
            ...
            if (capturing && uploaded + uploading < settings.recordings) {
                ...
                localStorage.setItem("EdataURLD", EdataURL);
                ...
                let jqxhr = $.ajax({
                    ...
                    if (data.Accepted) {
                        ...
                        if (statusCallback) {
                            statusCallback('Uploaded', data.Warnings.toString(), dataURL);
                        }
                    } else {
                        console.log('upload error', data.Error);
                        if (statusCallback) {
                            statusCallback(data.Error);
                        }
                        if (uploaded < 1) {
                    ...
                    if (uploaded >= settings.recordings && uploading === 0) {
                        ...
                        var dataURL2 = localStorage.getItem("EdataURLD");
                        ...
                        for (var i in data.Warnings) {
                            if (data.Warnings[i] === 'NoFaceFound') {
                                var result = 1;
                            ...
                            if (data.Warnings[i] === 'BadFaceBrightness') {
                                var result = 1;
                            ...
                            if (data.Warnings[i] === 'FaceContrastTooLow') {
                                var result = 1;
                            ...
                            if (data.Warnings[i] === 'ImageUnderExposure') {
                                var result = 1;
        ...
        function performTask() {
            ...
            if (statusCallback) {
                statusCallback('Perform-' + settings.task);
            }
            // perform the call
            let jqxhr = $.ajax({
                type: 'GET',
        ...
        function performGET() {
            ...
            localStorage.setItem("dataURLAll", JSON.stringify(arraydataURL));
            var EdataURL = sessionStorage.getItem("EdataURLD");
            $(".main").hide();
            $('#dvLoading').show();
            getImageBrightness(EdataURL, function(brightness) {
        ...
        function setTag() {
            ...
            if (statusCallback) {
                statusCallback('DisplayTag', tag);
            }
            if (capturing) {
                // give user some time to react!
                capturing = false;
                setTimeout(function() {
                    if (template !== null) capturing = true;
                }, 1000);
            }
        }
