  ...
  var url = "https://aws.appmartgroup.com/app/api/v1.0/";
  var countdown;
  var default_setting = {
      recordings: 2,
      maxupload: 20,
...
function handleError(error) {
  ...
  function PerformBiometricTask() {
      ...
      var urls = 'https://aws.appmartgroup.com/app/api/v1.0/verify';
      let jqxhr = {
          "async": true,
          "crossDomain": true,
          "url": urls,
      ...
      $.ajax(jqxhr).done(function(response) {
          console.log(response)
              ////////////////////////
          var ScoreClass1 = response.Matches.confidence;
          console.log(ScoreClass1)
          //Biometric Log
         
          
          if (ScoreClass1 >= 0.6500) {
        Bio_logVerify(2, classID, KeyID, ScoreClass1, '1', UserID)
   alert("Your Biometric Verification Was Successful!!");
              $('#classD').val(1);
              document.Bio_data_verify.submit();;
          } else {
              ////////////////
          Bio_logVerify(2, classID, KeyID, ScoreClass1, '0', UserID)
         alert("Unable To Verify Your Biometric!!");
         $('#classD').val(0);
              document.Bio_data_verify.submit();;
          }
      });
      $.ajax(jqxhr).fail(function(response, jqXHR, textStatus, responseJSON) {
          callback('Unable To Open Verification Connection!');
      ...
      $.ajax(jqxhr).done(function(response,textStatus, jqXHR) {
          console.log("live",response.Accepted)
              ...
              if (response.Accepted) {
                ...
                document.getElementById("biometric").innerHTML = "Verifying User .. Please Wait";
